...
void GameView::draw(sf::RenderWindow &window)
{
	...
	{
			Missile missile = l->getMissiles()[i];
			MissileGUI missileGUI = MissileGUI(missile);
			missileGUI.draw(window);
	}
	player.draw(window);
...
void GameView::getEnemies(std::vector<EnemyGUI> &enemies, GameLogic &l)
{
		enemies.clear();
